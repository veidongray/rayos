...
struct generic_host_control {
	uint32_t cap; // 0x00: Host Capabilities (主机能力寄存器)
	              //       - Bit 0~4 : NP (Number of Ports)
	              //       该芯片支持的最大端口数-1
	              //       - Bit 8~12: NCS (Number of Command Slots)
	              //       每个端口支持的命令槽数-1
	              //       - Bit 31  : 64位寻址能力 (1=支持64位物理地址)

	uint32_t
	        ghc; // 0x04: Global Host Control (全局主机控制寄存器)
	...
	uint32_t bohc; // 0x28: BIOS/OS Handoff Control and Status (BIOS/OS
	               // 软硬件交接控制状态寄存器)
...
struct sata_controller_port_register {
	// === DMA 基地址寄存器 ===
	uint32_t PxCLB; // 0x00: Command List Base Address (命令列表基物理地址 -
	                // 低32位)
	                //       - 必须 1024 字节对齐！
	uint32_t PxCLBU; // 0x04: Command List Base Address Upper
	                 // (命令列表基物理地址
	                 // - 高32位)

	uint32_t PxFB; // 0x08: FIS Base Address (接收FIS缓冲区基物理地址 -
	               // 低32位)
	               //       - 必须 256 字节对齐！
	uint32_t PxFBU; // 0x0C: FIS Base Address Upper (接收FIS缓冲区基物理地址
	                // - 高32位)
	...
	uint32_t PxIE; // 0x14: Interrupt Enable (端口中断允许寄存器)
	               //       - Bit 0: DHRE (Device to Host Register FIS)
	               //       收到硬盘响应时中断
	               //       - Bit 5: DPE (Descriptor Processed)
	               //       DMA数据搬运完成时中断

	// === 状态与控制 (重要) ===
	uint32_t
	        PxCMD; // 0x18: Command and Status (端口命令与状态寄存器)
	               //       - Bit 15: CR (Command List Running)
	               //       硬件命令列表引擎正在运转
	               //       - Bit 14: FR (FIS Receive Running)
	               //       硬件FIS接收引擎正在运转
	               //       - Bit 4 : FRE (FIS Receive Enable)
	               //       允许接收来自硬盘的数据包（点名和读写前必须开启）
	               //       - Bit 0 : ST (Start)
	               //       启动当前端口的DMA状态机引擎（筑巢完成后开启）

	uint32_t reserved0; // 0x1C: 保留空间

	uint32_t PxTFD; // 0x20: Task File Data (任务文件数据寄存器)
	                //       - 映射传统ATA状态。Bit 7 (BSY)
	                //       为1代表硬盘忙，Bit 0 (ERR) 为1代表出错

	uint32_t PxSIG; // 0x24: Signature (端口特征签名寄存器)
	                //       - 握手成功后，硬件自动填写设备类型
	                //       - 0x00000101 = SATA 硬盘, 0xEB140101 = SATAPI
	                //       光驱

	uint32_t PxSSTS; // 0x28: SATA Status (SATA 物理层状态寄存器 - 核心)
	                 //       - Bit 0~3 : DET (Device Detection)
	                 //       3=检测到设备且物理层通信成功
	                 //       - Bit 8~11: IPM (Interface Power Management)
	                 //       1=设备处于活跃供电状态

	uint32_t
	        PxSCTL; // 0x2C: SATA Control (SATA 物理层控制寄存器)
	                //       -
	                //       可以通过往此寄存器写值来强制触发当前端口线路的物理复位

	uint32_t
	        PxSERR; // 0x30: SATA Error (SATA 错误寄存器)
	                //       -
	                //       记录物理层各种电气和协议报错。初始化时**必须写入
	                //       0xFFFFFFFF 刷清**
	...
	uint32_t PxCI; // 0x38: Command Issue (命令发出寄存器 - 扔飞镖的地方)
	               //       - 32位图对应32个命令槽。往 Bit 0 写 1
	               //       意味着告诉硬件：“Slot 0
	               //       的读盘命令已准备好，立刻执行！”
	               //       - 硬件搬运完毕后，硬件会自动把这一位清零

	uint32_t PxSNTF; // 0x3C: SATA Notification (SATA 通知寄存器 -
	                 // 异步通知用)

	uint32_t PxFBS; // 0x40: FIS-based Switching Control
	                // (基于FIS切换控制寄存器)
 ...
 * HBA is an acronym for “host bus adapter”. A host bus adapter refers to the
 * silicon that implements the AHCI specification to communicate between system
 * memory and Serial ATA devices.
 */
struct hba_memory_registers {
	...
	uint8_t reserved_for_nvmhci
	        [64]; // 0x060 ~ 0x09F:
	              // 为NVMHCI(早期的NVMe规范草案)预留的空间

	uint8_t vendor_specific_registers
	        [96]; // 0x0A0 ~ 0x0FF:
	              // 芯片厂商(如Intel)自定义的扩展寄存器

	struct sata_controller_port_register
	        ports[32]; // 0x100 ~ 0x10FF: 32个完全平铺连续的端口寄存器阵列！
...
struct ahci_cmd_list_entry {
	...
	uint32_t ctba;  // Command Table Base Address (低 32
	                // 位物理地址，128字节对齐)
...
struct ahci_cmd_table {
	uint8_t cfis[64]; // 0x00 ~ 0x3F: 容纳各种类型的 CFIS 包 (包含上面的
	                  // fis_reg_h2d)
	uint8_t acmd[32];     // 0x40 ~ 0x5F: ATAPI 命令空间
	uint8_t reserved[32]; // 0x6F ~ 0x7F: 保留
	struct ahci_prdt_entry
	        prdt[128]; // 0x80 开始: 散集表阵列，支持多块内存片段
} __attribute__((packed));
