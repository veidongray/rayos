...
struct acpi_madt {
	...
	uint32_t local_apic_address; // 局部 APIC (Local APIC) 的物理基地址
	                             // (用于 APIC 模式)
	uint32_t flags; // 属性标志：Bit 0 置 1 表示系统支持双 8259
	                // 中断控制器兼容模式

	// 紧跟其后的是可变长度的 Local/IO APIC 结构体数组 (Interrupt Controller
	// Structures) 在 C 语言中可以用柔性数组（Flexible Array
	// Member）来表示，方便指针偏移
	uint8_t entries[];
} __attribute__((packed));
