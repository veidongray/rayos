...
static inline uint32_t cluster_to_lba(struct fat32_bpb *bpb, uint32_t cluster)
{
	uint32_t data_start_lba =
	        bpb->reserved_sector_count + (bpb->fat_size_32 * bpb->num_fats);
...
static inline size_t utf16_to_ascii(const uint16_t *utf16_str, char *ascii_str,
                                    size_t max_len)
{
	size_t i = 0;
...
{
	return (entry->sfn_entry.attr & 0x10) &&
	       entry->sfn_entry.name[0] == '.' &&
	       entry->sfn_entry.name[1] == ' ';
}
...
static inline int fat32_print_callback(struct lookup_context *ctx)
{
	...
	if ((uint8_t)ctx->entry->sfn_entry.name[0] != 0xE5) {
		if (ctx->entry->sfn_entry.attr & 0x10) {
			...
			printk("<-> %u %s", ctx->entry->sfn_entry.file_size,
			       ctx->lfn);
		}
	}
...
{
	if (ctx->lfn[0] == '\0') {
		sfn_to_ascii(ctx->entry->sfn_entry.name,
		             ctx->lfn); // 需要实现 SFN 转 ASCII 函数
	}
...
static inline uint32_t __fat32_alloc_cluster(void)
{
	...
	for (nr_fat = 0; (uint32_t)nr_fat < bpb->fat_size_32; nr_fat++) {
		ahci_read(port, bpb->reserved_sector_count + nr_fat, 1,
		          fat_table);
...
static inline size_t ascii_to_utf16(const char *ascii, uint16_t *utf16,
                                    size_t max_chars)
{
	size_t len = 0;
...
static inline int fat32_make_sfn_entry(struct fat32_dir_entry *entry,
                                       const char *fn)
{
	// 创建新的SFN
...
static inline int fat32_make_lfn_entry(struct fat32_dir_entry *entry,
                                       const char *fn, uint8_t order)
{
	int nr;
	uint16_t name[6];
...
int fat32_create(const char *path)
{
	...
	if (dir[0] == '/' && dir[1] == '\0') {
		...
		// 获取根路径的簇号
		dir_cluster = (entry_result.sfn_entry.first_cluster_hi << 16) +
		              entry_result.sfn_entry.first_cluster_lo;
	}
	...
	// 每簇可存放的目录项数量
	entries_per_cluster = bpb->bytes_per_sector * bpb->sectors_per_cluster /
	                      sizeof(struct fat32_dir_entry);
	while (!FAT32_EOC(dir_cluster)) {
		ahci_read(sata_dev->port, cluster_to_lba(bpb, dir_cluster),
		...
		for (uint32_t i = 0; i < entries_per_cluster; i++, entry++) {
			// 空目录项，目录结束
			if (((uint8_t)entry->sfn_entry.name[0] ==
			     SFN_NAME0_FREE) ||
			    ((uint8_t)entry->sfn_entry.name[0] ==
			     SFN_NAME0_DELETED)) {
				if (fat32_need_lfn(fn) && (fn_step >= fn)) {
					fat32_make_lfn_entry(
					        entry, fn_step,
					        ((nr_total_entries - nr_lfn) ==
					         1)
					                ? (LFN_ORDER_LAST_ENTRY |
					                   nr_lfn)
					                : nr_lfn);
		...
		// 下一簇
		ahci_read(sata_dev->port,
		          bpb->reserved_sector_count +
		                  (dir_cluster * sizeof(uint32_t) /
		                   bpb->bytes_per_sector),
		          1, fat_table);
		if (FAT32_EOC(fat_table[(
		            dir_cluster % bpb->bytes_per_sector %
		            (bpb->bytes_per_sector / sizeof(uint32_t)))])) {
			// 如果簇末尾了
			// 分配一个新的
			...
			ahci_read(sata_dev->port,
			          bpb->reserved_sector_count +
			                  (dir_cluster * sizeof(uint32_t) /
			                   bpb->bytes_per_sector),
			          1, fat_table);
			fat_table[(dir_cluster % bpb->bytes_per_sector %
			           (bpb->bytes_per_sector /
			            sizeof(uint32_t)))] = new_cluster;
			...
			ahci_write(sata_dev->port,
			           cluster_to_lba(bpb, dir_cluster),
			           bpb->sectors_per_cluster, cluster_buf);
		} else {
			// 直接从fat读取下一个
			dir_cluster = fat_table[(
			        dir_cluster % bpb->bytes_per_sector %
			        (bpb->bytes_per_sector / sizeof(uint32_t)))];
		}
	}
...
int fat32_read_cluster(struct fat32_file *fp, char *buf, size_t size)
{
	...
	cluster = (fp->fs.entry.sfn_entry.first_cluster_hi << 16) +
	          fp->fs.entry.sfn_entry.first_cluster_lo;
	bytes_per_cluster = bpb->bytes_per_sector * bpb->sectors_per_cluster;
	cluster_buf = kzalloc(bytes_per_cluster);
	while (!FAT32_EOC(cluster)) {
		ahci_read(port, cluster_to_lba(bpb, cluster),
		          bpb->sectors_per_cluster, cluster_buf);
		...
		ahci_read(port,
		          ...
		          1, fat_table);
		cluster =
		        fat_table[(cluster % bpb->bytes_per_sector %
		                   (bpb->bytes_per_sector / sizeof(uint32_t)))];
	}
...
int fat32_write_cluster(struct fat32_file *fp, const char *buf, size_t size)
{
	...
	clusbuff = kzalloc(fp->fs.bpb->bytes_per_sector *
	                   fp->fs.bpb->sectors_per_cluster);
	fat_table =
	        kzalloc(fp->fs.bpb->fat_size_32 * fp->fs.bpb->bytes_per_sector);
	entries_per_cluster = (fp->fs.bpb->bytes_per_sector *
	                       fp->fs.bpb->sectors_per_cluster) /
	                      32;
	if (((fp->fs.entry.sfn_entry.first_cluster_hi << 16) +
	     fp->fs.entry.sfn_entry.first_cluster_lo) == 0) {
		// 第一次写入，需要先找可用簇
		cluster = __fat32_alloc_cluster();
		fp->fs.entry.sfn_entry.first_cluster_hi =
		        (cluster & 0xffff0000) >> 16;
		fp->fs.entry.sfn_entry.first_cluster_lo = cluster & 0x0000ffff;

		// 写回目录项内容
		cluster =
		        (fp->fs.parent_entry.sfn_entry.first_cluster_hi << 16) +
		        fp->fs.parent_entry.sfn_entry.first_cluster_lo;
		while (!FAT32_EOC(cluster)) {
			// 查找对应的项的簇
			ahci_read(fp->fs.sata_dev->port,
			          cluster_to_lba(fp->fs.bpb, cluster),
			          fp->fs.bpb->sectors_per_cluster, clusbuff);
			entry = (struct fat32_dir_entry *)clusbuff;

			for (int i = 0; i < entries_per_cluster; i++, entry++) {
				if (strncmp(entry->sfn_entry.name,
				            fp->fs.entry.sfn_entry.name,
				            11) == 0) {
					fp->fs.entry.sfn_entry.file_size = size;
					memcpy(entry, &fp->fs.entry,
					       sizeof(struct fat32_dir_entry));
					ahci_write(
					        fp->fs.sata_dev->port,
					        cluster_to_lba(fp->fs.bpb,
					                       cluster),
					        fp->fs.bpb->sectors_per_cluster,
					        clusbuff);
					goto done;
			...
			// 读取当前 cluster 索引表
			ahci_read(fp->fs.sata_dev->port,
			          fp->fs.bpb->reserved_sector_count +
			                  (cluster * sizeof(uint32_t) /
			                   fp->fs.bpb->bytes_per_sector),
			          1, fat_table);
	...
	cluster = (fp->fs.entry.sfn_entry.first_cluster_hi << 16) +
	          fp->fs.entry.sfn_entry.first_cluster_lo;
	uint32_t bytes_per_cluster =
	        fp->fs.bpb->bytes_per_sector * fp->fs.bpb->sectors_per_cluster;
	uint32_t new_cluster;
	for (char *b = (char *)buf; b < (buf + size);
	     b +=
	     (fp->fs.bpb->bytes_per_sector * fp->fs.bpb->sectors_per_cluster)) {
		// 一次写入一个簇
		...
		ahci_read(fp->fs.sata_dev->port,
		          ...
		          1, fat_table);
		if (FAT32_EOC(
		            fat_table[(cluster % fp->fs.bpb->bytes_per_sector %
		                       (fp->fs.bpb->bytes_per_sector /
		                        sizeof(uint32_t)))])) {
			// 如果簇末尾了
			// 分配一个新的
 ...
 */
int fat32_foreach_dirent(fat32_foreach_dirent_callback_t cb,
                         uint32_t dir_cluster, const char *target,
                         struct fat32_dir_entry *result)
{
	...
	while (!FAT32_EOC(dir_cluster)) {
		ahci_read(port, cluster_to_lba(bpb, dir_cluster),
		          bpb->sectors_per_cluster, cluster_buf);
		entry = (struct fat32_dir_entry *)cluster_buf;
...
int fat32_lookup(const char *path, struct fat32_dir_entry *entry_result)
{
	...
	while ((dir = path_next(dir, name)) != NULL) {
		ret = fat32_foreach_dirent(fat32_compare_callback, cluster,
		                           name, &entry);
		if (ret < 0) {
			entry_result = NULL;
			kfree(name);
		...
		cluster = (entry.sfn_entry.first_cluster_hi << 16) +
		          entry.sfn_entry.first_cluster_lo;
	}
...
int fat32_readdir(const char *path)
{
	...
	if (path[0] == '/' && path[1] == '\0') {
		...
		if (entry.sfn_entry.attr & 0x10) {
			fat32_foreach_dirent(
			        fat32_print_callback,
			        (entry.sfn_entry.first_cluster_hi << 16) +
			                entry.sfn_entry.first_cluster_lo,
			        NULL, NULL);
		} else {
...
int fat32_open(const char *path)
{
	...
	if (ret < 0) {
		...
		if ((strlen(parent) == 1) && (strncmp(parent, "/", 1) == 0)) {
			...
			memcpy(&fp->fs.parent_entry, &parent_entry,
			       sizeof(struct fat32_dir_entry));
		}
