...
uint32_t __acpi_find_gsi_for_irq(uint8_t irq)
{
	...
	while (ptr < end) {
		...
		if (type == 2) {
			...
			printk("MADT ISO: BusSrc=%u IRQSrc=%u -> GSI=%u "
			       "Flags=%#x",
			       iso->bus_source, iso->irq_source, iso->gsi,
			       iso->flags);

			if (iso->irq_source == irq) {
				printk("MADT: IRQ%u remapped to GSI %u", irq,
				       iso->gsi);
				return iso->gsi;
			}
		}
	...
	printk("MADT: No ISO for IRQ%u, using identity mapping GSI=%u", irq,
	       irq);
	return irq;
}
...
void acpi_init(void)
{
	...
	if (rsdp->revision >= 2) {
		...
		xsdt = (struct acpi_xsdt *)((uint64_t)rsdp->xsdt_address +
		                            KERNEL_BASE);

		/* 映射 XSDT 到虚拟地址空间 */
		map_page((uint64_t)rsdp->xsdt_address, (uint64_t)xsdt,
		         MAP_KERN_MMIO);
	} else {
		/* 映射 RSDT */
		rsdt = (struct acpi_rsdt *)((uintptr_t)rsdp->rsdt_address +
		                            KERNEL_BASE);
		map_page((uint64_t)rsdp->rsdt_address, (uint64_t)rsdt,
		         MAP_KERN_MMIO);

		printk("map rsdt_address %#llx -> %#llx", rsdp->rsdt_address,
		       rsdt);

		/* 计算 RSDT 中表项数量 */
		uint32_t nr_rsdt_entry = (rsdt->header.length -
		...
		for (uint32_t count = 0; count < nr_rsdt_entry; count++) {
			struct acpi_sdt_header *header =
			        (struct acpi_sdt_header
			                 *)((uintptr_t)rsdt->entries[count] +
			                    KERNEL_BASE);

			/* 映射每一个 ACPI 表 */
			map_page((uint64_t)rsdt->entries[count],
			         (uint64_t)header, MAP_KERN_MMIO);
			...
			if (!strncmp(header->signature, "MCFG", 4)) {
				...
				printk("MCFG number of tables %u",
				       (mcfg->header.length -
				        sizeof(struct acpi_sdt_header) - 8) /
				               16);
			}
		}
	}
