...
__attribute__((aligned(
        1024))) static struct ahci_cmd_list_entry cl_buffer[32]; // 32个Slot
__attribute__((aligned(4096))) static struct ahci_cmd_table
        cmd_table_buffer; // 升级为页级命令表
...
struct sata_device *get_sata_device(void) { return sata_dev; }

struct hba_memory_registers *get_host_bus_adapter(void) { return hba; }

int ahci_dma_transfer(int write, struct sata_controller_port_register *port,
                      uint64_t lba, uint16_t count, void *target_buf_virt)
{
	/*
	[HBA Port 寄存器]
	     │
      ...
      │     ├── prdt[0] ──► [物理地址 DBA] + [操作字节数 DBC] ───┼─► 丢进内核的
      sector_data │     └── prdt[1] ──► [物理地址 DBA] + [操作字节数 DBC] ───┼─►
      (若有多块内存，继续往后接...)
      └──────────────────────────────────────────────────────────┘
	*/
	...
	cmd_table_buffer.prdt[0].dbau =
	        (uint32_t)((target_phys >> 32) & 0xFFFFFFFFU);
	cmd_table_buffer.prdt[0].reserved0 = 0;
	// 传输控制：(字节数 - 1) | 传输完毕中断允许 (Bit 31)
	cmd_table_buffer.prdt[0].dbc =
	        ((count * 512 - 1) & 0x3FFFFFU) | (1U << 31);
...
int ahci_read(struct sata_controller_port_register *port, uint64_t lba,
              uint16_t count, void *target_buf_virt)
{
	return ahci_dma_transfer(0, port, lba, count, target_buf_virt);
}

int blkdev_ahci_read(struct block_device *blkdev, uint64_t lba, uint32_t count,
                     void *target_buf_virt)
{
	return ahci_dma_transfer(
	        0,
	        &((struct ahci_priv *)blkdev->priv)
	                 ->hba
	                 ->ports[((struct ahci_priv *)blkdev->priv)->portno],
	        lba, count, target_buf_virt);
}

int ahci_write(struct sata_controller_port_register *port, uint64_t lba,
               uint16_t count, void *target_buf_virt)
{
	return ahci_dma_transfer(1, port, lba, count, target_buf_virt);
}

int blkdev_ahci_write(struct block_device *blkdev, uint64_t lba, uint32_t count,
                      void *target_buf_virt)
{
	return ahci_dma_transfer(
	        1,
...
struct block_device_ops blkops = {.read = blkdev_ahci_read,
...
void ahci_init(uintptr_t ahci_base)
{
	...
	hba = (struct hba_memory_registers *)((uint64_t)ahci_base +
	                                      KERNEL_BASE);
	map_page_range((uint64_t)ahci_base, (uint64_t)ahci_base + KERNEL_BASE,
	               MAP_KERN_MMIO,
	               (sizeof(struct hba_memory_registers) >> PAGE_SHIFT) + 1);
	...
	for (size_t i = 0; i < nr_ports; i++) {
		...
		if (((hba->ports[i].PxSSTS & 0xfU) == 0x3U) &&
		    (((hba->ports[i].PxSSTS & 0xf00U) >> 8) == 0x1U)) {
			sata_dev_t dev_type =
			        ahci_check_device_type(hba->ports[i].PxSIG);
			if (dev_type == SATA_DEV_SATA) {
				printk("[Port %d] -> Found SATA Hard Disk "
				       "(HDD/SSD).",
				       i);
				...
				uint64_t fis_phys =
				        get_physaddr((uint64_t)fis_buffer);
				hba->ports[i].PxFB =
				        (uint32_t)(fis_phys & 0xFFFFFFFFU);
				hba->ports[i].PxFBU =
				        (uint32_t)((fis_phys >> 32) &
				                   0xFFFFFFFFU);
				memset(fis_buffer, 0, 256);

				// 分配并配置 Command List 物理基地址
				uint64_t cl_phys =
				        get_physaddr((uint64_t)cl_buffer);
				hba->ports[i].PxCLB =
				        (uint32_t)(cl_phys & 0xFFFFFFFFU);
				hba->ports[i].PxCLBU =
				        (uint32_t)((cl_phys >> 32) &
				                   0xFFFFFFFFU);
				...
				uint64_t ctba_phys = get_physaddr(
				        (uint64_t)&cmd_table_buffer);
				memset(&cmd_table_buffer, 0,
				       sizeof(struct ahci_cmd_table));

				cl_buffer[0].ctba =
				        (uint32_t)(ctba_phys & 0xFFFFFFFFU);
				cl_buffer[0].ctbau =
				        (uint32_t)((ctba_phys >> 32) &
				                   0xFFFFFFFFU);
				...
				if ((hba->ports[i].PxCMD & (1U << 15)) &&
				    (hba->ports[i].PxCMD & (1U << 14))) {
					sata_dev = kmalloc(
					        sizeof(struct sata_device));
					sata_dev->hba = hba;
					sata_dev->port_no = i;
					sata_dev->port = &hba->ports[i];

					struct ahci_priv *priv =
					        (struct ahci_priv *)kzalloc(
					                sizeof(struct
					                       ahci_priv));
					priv->cl_buffer = cl_buffer;
					priv->cmd_table_buffer =
					        &cmd_table_buffer;
					priv->fis_buffer = fis_buffer;
					priv->hba = hba;
					priv->portno = i;
					struct block_device *blkdev =
					        (struct block_device *)kzalloc(
					                sizeof(struct
					                       block_device));
					sprintf(blkdev->info.bd_name, "sata%d",
					        i);
					blkdev->priv = priv;
					blkdev_register(blkdev, &blkops);
				} else {
					printk("[Port %d] -> DMA Engine failed "
					       "to start!",
					       i);
				}
			} else if (dev_type == SATA_DEV_SATAPI) {
				printk("[Port %d] -> Found SATAPI CD-ROM. "
				       "Ignore for now.",
				       i);
			} else {
				printk("[Port %d] -> No valid SATA device or "
				       "device busy (SIG: 0x%08X).",
				       i, hba->ports[i].PxSIG);
			}
		}
	}
