...
{
	__u64 rte = (__u64)vector | RTE_DELIVERY_FIXED | RTE_DEST_PHYSICAL |
	            flags | ((__u64)dest_apic_id << RTE_DEST_SHIFT);
...
void ioapic_init(void)
{
	...
	printk("IOAPIC base %#llx map to %#llx", ioapic_base,
	       (__u8 *)ioapic_base + KERNEL_BASE);
	map_page((__u64)ioapic_base, (__u64)ioapic_base + KERNEL_BASE,
	         MAP_KERN_MMIO);
	...
	ioapic_enable_irq(com1_gsi, X86_IRQ_COM1, curr_lapic_id,
	                  RTE_EDGE_TRIGGERED | RTE_ACTIVE_HIGH);
}