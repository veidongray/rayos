...
size_t memused(void) { return bitmap_count_set(&page_alloc_bitmap) * 4096; }
...
void page_init(void)
{
	...
	bitmap_set_range(
	        &page_alloc_bitmap, 0,
	        ((uint64_t)_kernel_phys_end_aligned + bitmap_data_bytes) >>
	                PAGE_SHIFT);
	// unmap unused page
	unmap_page_range((uint64_t)_kernel_virt_end_aligned + bitmap_data_bytes,
	                 (BOOTMAP_LEN - (uint64_t)_kernel_phys_end_aligned -
	                  bitmap_data_bytes) >>
	                         PAGE_SHIFT);
}
...
uint64_t alloc_pages(size_t order)
{
	...
	ret = bitmap_alloc_range(&page_alloc_bitmap, order_to_pages(order),
	                         start);
	if (ret < 0) {
		return ret;
	}
...
{
	bitmap_free_range(&page_alloc_bitmap, physaddr >> PAGE_SHIFT,
	                  order_to_pages(order));
}

int map_page_range(uint64_t physaddr, uint64_t virtaddr, uint64_t flags,
                   size_t len)
{
	size_t i, j;
	uint64_t va, pa;
	...
	for (i = 0; i < len; i++, pa += 0x1000, va += 0x1000) {
		...
		// 所有地址均为 canonical（高16位全1），适用于 x86-64
		// 递归页表映射
		map_pml4 = (uint64_t *)(PML4_BASE << 12ULL);
		map_pdpt = (uint64_t *)((PML4_BASE << 21ULL) +
		                        (pml4_idx << 12ULL));
		map_pd =
		        (uint64_t *)((PML4_BASE << 30ULL) +
		                     (pml4_idx << 21ULL) + (pdpt_idx << 12ULL));
		map_pt = (uint64_t *)((PML4_BASE << 39ULL) +
		                      (pml4_idx << 30ULL) +
