...
| `help` | 显示帮助信息 |
| `check-deps` | 检查软件依赖 |
| `build` | 编译内核并生成可执行文件（支持并行构建） |
| `disk` | 创建 FAT32 磁盘镜像（增量更新） |
| `iso` | 生成可引导 GRUB ISO |
| `qemu` | 在 QEMU 中运行系统 |
| `qemudbg` | 以调试日志模式运行 QEMU |
| `qemugdb` | 运行 QEMU 并等待 GDB 连接 |
| `clean` | 清除除磁盘镜像外的所有构建产物 |
| `distclean` | 清除所有构建产物（含用户程序和磁盘镜像） |
| `rebuild` | 清理并从头重新构建 |
| `format` | 格式化所有 C/C++ 源文件 |
...
您可以自由地对本项目进行使用、复制、修改、合并、出版发行、散布、再授权及贩售，包括用于商业项目。唯一条件是：在软件的所有副本或重要部分中，必须包含上述版权声明和本许可声明。

本软件按“原样”提供，不提供任何明示或暗示的保证。详情请参阅 [LICENSE](LICENSE) 文件。
